...
Header `‹ DEPOT` + sync. Title = gear name; meta mono `ITEM · SHARED · ×2 · 1.9 KG EACH` — the weight segment is tagged **LATER** (story 16); the MVP line is `ITEM · SHARED · ×2` (variant drawn beside the frame). Tag chips (32px, bordered).
...
- `Components.dc.html` — component sheet: rows, chips, per-person indicators, filters, containment tree, quantity controls, empty/loading states, packing actions; §09 auth atoms (passkey/device rows, person-with-login indicator, sign-in button states, expiry chip, invite card, auth explainer sheet); §10 the LATER scope tag.
...
- `Screens B - Detail + Trips + Find + Unpack.dc.html` — Gear detail, Trips, Gear list builder (desktop) + MVP header/footer variants, over-claim & reopen sheets (§02B), Find, Unpack & close.
