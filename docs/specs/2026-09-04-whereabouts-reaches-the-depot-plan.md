...
- [ ] **Step 3: The memo.** `const TRIP_SLICES = new WeakMap<HouseholdState, { byGear: Map<string, TripSliceFacts[]>; overClaimed: ReadonlySet<string> }>()`, built by one pass over `visibleTrips(state).filter(isActive)` — one `tripContainmentView` per Trip, `overClaims(state)` read **once** into the set. Model the docstring on `slice.ts`'s `tripMembershipOf`, which states the whole argument; spec §3.5 says this is its second instance.
...
- [ ] **Step 2: Run them** and watch them fail.
- [ ] **Step 3: The ancestors memo.** `const CONTAINER_ANCESTORS = new WeakMap<HouseholdState, Map<string, readonly string[]>>()`, each Gear's **container** ancestors outermost-first, built from one `containmentView(state)`. Its docstring gives spec §3.5's reason — `valuesOf` gets no view, and a per-row `containmentView` is O(n² log n) on the app's most-visited screen — and names the rejected alternative (memoising `containmentView` itself) with its reason.
...
- [ ] **Step 5: The grouping row.** `keyOf` reads **the same ancestors memo** — the ancestors are outermost-first, so the **last** element is the immediate container, and an empty list is the sentinel. `Grouping.keyOf` receives `(gear, state)` and no view, which is exactly why it must not build one; sharing the memo is also what keeps the filter and the group from ever disagreeing about what contains what. The immediate container is the only thing a partition can be. `format` uses the identical words as the dimension (D4: one bucket, one name). `pinned: NOT_IN_A_CONTAINER`. Add `'container'` to `GROUP_KEYS`.
...
- Consumes: Task 2's `whereabouts`, `whereaboutsText`, `sliceCountLabel`, `whereaboutsByPerson`, `Whereabouts`, `WhereaboutsSlice`; `sortedPeople` (`app/src/household/people.ts`); `ui/`'s `PersonCircle` at `size={22}`.
...
- [ ] **Step 2: Run them** and watch them fail.
- [ ] **Step 3: `WhereaboutsCard`.** Props stay **presentational** ([frontend-design §5](../frontend-design.md): takes domain data, reads no store, imports no selector): `slices: readonly WhereaboutsSlice[]` and `overClaim?: { text: string; href: string }`. **`GearDetail` composes that text**, because §6.1's Counted-only rule is a domain rule and this component is the one waiting on a second caller to move into `ui/`. **Whether the gear splits is derived, not passed** — it is `slices.some((s) => sliceCountLabel(s) !== null)`, which is already D1's rule and needs no second spelling. Key rows by `` `${slice.kind}:${slice.kind === 'trip' ? slice.tripId : ''}` ``. Delete the JSDoc paragraph that logs the collision — **this component closes it**, and a docstring describing a fixed defect is worse than none. The ▲ footer's two numbers are **Counted-only** (spec §6.1); every other Kind reads `▲ CLAIMED BY N TRIPS`.
